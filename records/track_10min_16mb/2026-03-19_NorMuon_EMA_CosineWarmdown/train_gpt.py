...
# NORMUON OPTIMIZER
# -----------------------------
# NorMuon: Muon with per-row adaptive step sizes via second moment estimation.
# From https://github.com/zichongli5/NorMuon, popularized by modded-nanogpt.
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: float, mlp_hidden: int = 0):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float, rope_base: float,
                 qk_gain_init: float, mlp_hidden: int = 0):
        super().__init__()
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        qk_gain_init=args.qk_gain_init, mlp_hidden=args.mlp_hidden,
    ).to(device).bfloat16()
    ...
    log0(f"optimizer:NorMuon momentum:{args.muon_momentum} beta2:{args.muon_beta2}")
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        # Cosine warmdown: smoother decay than linear, holds higher LR longer.
    ...
    log0(f"ema:initialized decay={args.ema_decay} start_step={args.ema_start_step}")
    ...
    # --- APPLY EMA WEIGHTS ---
    log0("applying EMA weights to model")
